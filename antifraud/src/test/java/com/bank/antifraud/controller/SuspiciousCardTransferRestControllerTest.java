...
@WebMvcTest(controllers = {SuspiciousCardTransferRestController.class})
@ExtendWith(MockitoExtension.class)
class SuspiciousCardTransferRestControllerTest<T extends SuspiciousTransfer, R extends SuspiciousTransferDto> {
    ...
    void getAllShouldReturnStatusOk() throws Exception {
        ...
        when(suspiciousCardTransferService.findAll())
                .thenReturn((List<SuspiciousTransferDtoImpl>) result);
        mockMvc.perform(MockMvcRequestBuilders.get("/transfer/cards")
    ...
    void getTransferShouldReturnStatusOk() throws Exception {
        R result = (R) new SuspiciousTransferDtoImpl(777L,
                true, true,
                "Oops", "No");
        when(suspiciousCardTransferService.findById(777L))
                .thenReturn((SuspiciousTransferDtoImpl) result);
        mockMvc.perform(MockMvcRequestBuilders.get("/transfer/cards/777")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void addTransferShouldReturnStatusOk() throws Exception {
        ...
        var response = new SuspiciousCardTransfer();
        response.setId(1L);
        response.setCardTransferId(777L);
        ...
        when(suspiciousCardTransferService.save(any()))
                .thenReturn(response);

        String json = new ObjectMapper().writeValueAsString(response);
        mockMvc.perform(MockMvcRequestBuilders.post("/transfer/cards")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", Matchers.is(1)))
                .andExpect(jsonPath("$.cardTransferId", Matchers.is(777)))
    ...
    void updateTransferShouldReturnStatusOk() throws Exception {
        ...
        when(suspiciousCardTransferService.update(any(), any()))
                .thenReturn(response);

        String json = new ObjectMapper().writeValueAsString(response);
        mockMvc.perform(MockMvcRequestBuilders.put("/transfer/cards/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isOk());
    }
    ...
    void deleteTransferShouldReturnStatusOk() throws Exception {
        ...
        when(suspiciousCardTransferService.delete(any()))
                .thenReturn(response);

        mockMvc.perform(MockMvcRequestBuilders.delete("/transfer/cards/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
